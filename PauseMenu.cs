...
public class PauseMenu : MonoBehaviour
{
	...
	public bool paused = false, despausou = false;
    ...
    string[] textoBonus = { "GO", "GL", "MWhaha", "HF", "GG", "DIE" };
    private int random;
    void Start ()
	{
	...
	void PosicoesMenu(){
        ...
        if (!mouseOver)
        {
            Debug.Log("chamou " + botoes[posicao]);
    ...
    void Volta ()
	{
        ...
        if (tempoVolta > 3.8f && tempoVolta < 5)
        {
            Time.timeScale = 0.1f;
            tempo.SetActive(true);
            
            tempo.GetComponent<Text>().text = "" + textoBonus[random].ToString();
        }
        if (tempoVolta >= 5) {
			Time.timeScale = 1;
			tempo.SetActive (false);
